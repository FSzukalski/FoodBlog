...
const Favorites = ({ favoritesList, setfavoritesList }) => {
  ...
  return (
    <Wrapper>
      <h1 style={{ textAlign: 'center', textShadow: '2px 2px 4px #000000' }}>My favorites</h1>
      <Splide
        ...
        {favoritesList.map((favorite, index) => (
          <SplideSlide key={index}>
            <Card>
              ...
              <h3 style={{ textAlign: 'center', textShadow: '2px 2px 6px #000000' }}>{favorite.name}</h3>

              <center><h4>Ingredients:</h4></center>

              {favorite.data.ingredientLines.map(element => { return <li>{element}</li> })}


              <a href={favorite.data.url}><button>Link to recipe</button></a>
